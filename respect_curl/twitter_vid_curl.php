...
$url= "";
$max_bitrate = 0;
$max_bitrate_key = null;
$mp4_count = 0;
foreach($variants as $key=>$val){
    if($val['content_type'] == "video/mp4" and $max_bitrate < $val['bitrate']){
        $max_bitrate = $val['bitrate'];
        $max_bitrate_key = $key;
        $mp4_count ++;
    }
}
// get rid of the largest one (only if there are more than 1 ) and go for the next largest one
if($mp4_count>1) unset($variants[$max_bitrate_key]);
$max_bitrate = 0;
foreach($variants as $val){
    if($val['content_type'] != "video/mp4") continue;
    if($max_bitrate < $val['bitrate']){
        $max_bitrate = $val['bitrate'];
        $url = $val['url'];
    }
}
...
echo $url;
if(!$url){
    echo "FETCH URL FAIL !!!".PHP_EOL;
    die;
}
$cmd_curl_dl = "curl -o /tmp/$tid.mp4 -v \"$url\" --proxy \"https://www.thebiotek.com:4443\" --proxy-user \"thebiotek:Pass.1973\"";
shell_exec($cmd_curl_dl);
